...
; PRO mt_ratio - dialog for calculating a ratio- or logratio-image
;			   - different kinds can be selected
; module for calculating a ratio-image
;------------------------------------------------------------------------
...
COMMON rat, types, file, wid, config
	...
	; main-widget
	main = WIDGET_BASE(GROUP_LEADER = wid.base, /COLUMN, TITLE = 'Create ratio image',/floating,/tlb_kill_request_events,/tlb_frame_attr)
	...
	selector = 0
	select_but = CW_BGROUP(base_select, ['Ratio - (X / Y)', 'Ratio - min((X / Y), (Y / X))'], SET_VALUE = selector, /NO_RELEASE, /EXCLUSIVE)

	; pixel or box
	box_label = WIDGET_LABEL(base_piorbo, VALUE = 'Pixel or Box')
	pixorbox = 0
	piorbo = CW_BGROUP(base_piorbo, ['Ratio between pixels', 'Ratio between mean of box', 'Blockratio (mosaic)'], SET_VALUE = pixorbox, /NO_RELEASE, /EXCLUSIVE)
	boxsize = 3
	...
	REPEAT BEGIN
		...
		IF master EQ slave THEN BEGIN
			infotext = ['The ratio between equal', 'files or layers is 1.', 'Control your selection!']
			notice = DIALOG_MESSAGE(infotext, DIALOG_PARENT = main, TITLE = 'Information')
		ENDIF
	...
	srat, outputfile, outfile, header = [2l, file.xdim, file.ydim, 4l], type = LONG(122), info = 'Ratio image ('+types[type]+')'
	...
	; window for status
	progress, MESSAGE = 'Calculating ratio ...'
...
; ------- FILTER ------- BEGIN
		...
		; difference y-x
		array_out = array_in1 / array_in2

		; find infinite elements
		posi = WHERE(FINITE(array_out) EQ 0, nr)
		IF nr GT 0 THEN array_out[posi] = 0.0

		; find minimum (faster than pixel for pixel)
		IF selector EQ 1 THEN BEGIN
			posi = WHERE(ABS(array_out) GT 1.0, nr)
			IF nr GT 0 THEN array_out[posi] = 1 / array_out[posi]
		ENDIF
...
; ------- FILTER ------- END
	...
	file.info = 'Ratio image ('+types[type]+')'
	file.type = LONG(122)
	file.mult = 1l
	file.var = 4l
