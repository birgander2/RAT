...
pro polin_opt_sb,CALLED = called, METHOD=METHOD, SM_FILE=sm_file, SMMX = smmx, SMMY = smmy
  ...
  if n_elements(sm_file) eq 0 then sm_file = file_dirname(file.name,/M)+'sm_sb_'+(method?'esm_':'msm_')+file_basename(file.name)
...
; read / write header
  ...
  if sms then begin
     smtype = 535L
     if method then $
        smhead = [6L,pol,pol,1,n_bl,head[3:*]] $
     else smhead = [6L,pol,pol,2,n_bl,head[3:*]]
...
; pop up progress window
  progress,Message='Single-baseline coherence optimization...',/cancel_button
...
;start block processing
  for i=0,anz_blocks-1 do begin ; loop normal blocks
     ...
     oblock = make_array([pol,n_bl,file.xdim,blocksizes[i]],type=file.var,/nozero)
     if sms then $
        smblock= make_array([pol,pol,2-method,n_bl,file.xdim,blocksizes[i]],type=file.var,/nozero)
     readu,ddd,block
...
; -------- THE FILTER ----------
     writeu,eee, oblock
     if sms then writeu,fff,smblock
  endfor
  free_lun,ddd,eee
...
; update file information
  ...
  evolute,'Single-baseline coherence optimization with '+(method eq 0?'SB-MSM':'SB-ESM')
