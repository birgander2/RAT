...
function polin_coreg_amp,arr1,arr2,k
  compile_opt idl2
...
pro polin_coreg_one,CALLED = called, OFFSETX = offsetx, OFFSETY = offsety, channel=ch
  common rat, types, file, wid, config
  common polin_info
  compile_opt idl2
  
  if (file.type lt 500 && file.type gt 514) then begin
    error = DIALOG_MESSAGE("This is not a PolInSAR data set.", $
                           DIALOG_PARENT = wid.base, TITLE='Error',/error)
    return
  endif
  polin_get_info_common
  if matrix then begin
    error = DIALOG_MESSAGE("Expects scattering vector form (not matrix)!", $
                           DIALOG_PARENT = wid.base, TITLE='Error',/error)
    return
  endif
  if n_tr ne 2 then begin
    error = DIALOG_MESSAGE("Expects exactly 2 tracks!", $
                           DIALOG_PARENT = wid.base, TITLE='Error',/error)
    return
  endif
  ...
  if n_elements(ch) eq 0 then ch = 0 ;; initialization channel
  if n_elements(offset_method) eq 0 then offset_method = 0
  
  if matrix $
  then ch_groups=string(mm_v2m(indgen(pol))+mm_v2m(indgen(pol)*10,/tr),f='(i02)') $
  else ch_groups=string(indgen(pol),f='(i1)')
  ch_rows = matrix? pol: 1
  
  if not keyword_set(called) then begin ; Graphical interface
    main = WIDGET_BASE(GROUP_LEADER=wid.base,/column,TITLE='Coarse PolInSAR image registration',/floating,/tlb_kill_request_events,/tlb_frame_attr)
    ...
    dummy  = widget_base(main,/column,/frame)
    ch_wid = CW_BGROUP(dummy,ch_groups[*],/exclusive,set_value=ch, row=ch_rows, $
                       label_top='Choose polarization channel')
    ...
    repeat begin                ; Event loop
      ...
      if event.id eq but_info then begin ; Info Button clicked
        infotext = ['COARSE IMAGE COREGISTRATION',$
                    ...
                    'and extended to multiple channels 2010 by Maxim Neumann']
        info = DIALOG_MESSAGE(infotext, DIALOG_PARENT = main, TITLE='Information')
      end
      if event.id eq ch_wid then $
        widget_control, ch_wid, get_value=ch
      if (event.id eq but_estimate) then begin
        ...
        arr = make_array([2,bsx,bsy],type=file.var)
        for i=0l,bsy/bsx-1 do begin
          ...
          if matrix then begin
            arr[0,*,i*bsx:i*bsx+bsx-1] = reform((dummy[0:pol-1,0:pol-1,*,*])[ch mod pol,ch/pol,*,*])
            arr[1,*,i*bsx:i*bsx+bsx-1] = reform((dummy[pol:2*pol-1,pol:2*pol-1,*,*])[ch mod pol,ch/pol,*,*])
          endif $
          else arr[*,*,i*bsx:i*bsx+bsx-1] = reform(dummy[ch,*,*,*])
        endfor
...
; -------- THE FILTER ----------
    if matrix then begin
      ...
      for p=0,pol-1 do begin
        block[p,1,*,*] = shift(block[p,1,*,*],0,0,offsetx,offsety)
        if offset_method eq 1 then begin
          if offsetx gt 0 then block[p,*,0:offsetx,*]=0
          if offsetx lt 0 then block[p,*,file.xdim-offsetx:*,*]=0
          if offsety gt 0 && total(blocksizes[0:i]-overlap)-(blocksizes[i]-overlap) lt offsety then $
            block[p,*,*,0:0>(offsety-1-(total(blocksizes[0:i]-overlap)-(blocksizes[i]-overlap)))<(ypos2-1)]=0
          if offsety lt 0 && total(blocksizes[i:*]-overlap)-(blocksizes[i]-overlap) lt -offsety then $
            block[p,*,*,0>(offsety-overlap+1+total(blocksizes[i:*]-overlap))<(ypos2-1):*]=0
        endif
      endfor
    endelse
; -------- THE FILTER ----------
...
; update file information
  ...
  evolute,'Coarse interferometric PolInSAR coregistration with offsets '+strcompress([offsetx,offsety])+' and pixel offset handling method '+strcompress(offset_method)  
