...
pro wiz_k2wclass
	...
	main = widget_base(GROUP_LEADER=wid.base,title='WIZARD: Scattering vector -> H/a/A-Wishart classification',row=3,/floating,/tlb_kill_request_events,/tlb_frame_attr)
...
; start batch
	...
	c_to_t,/called
	if wid.cancel eq 1 then return
	file_copy,file.name,config.tempdir+'tmp_T.rat',/overwrite

	decomp_entalpani,/called
	if wid.cancel eq 1 then return
	file_copy,file.name,config.tempdir+'tmp_eaa.rat',/overwrite

	classif_wishart_opt, tfile=config.tempdir+'tmp_T.rat',cfile=config.tempdir+'tmp_eaa.rat',/called
	if wid.cancel eq 1 then return

; remove temporary files
	
	file_delete,config.tempdir+'tmp_T.rat'
	file_delete,config.tempdir+'tmp_eaa.rat'
