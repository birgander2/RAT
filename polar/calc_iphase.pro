...
; All Rights Reserved.
;------------------------------------------------------------------------
pro calc_iphase,CALLED = called,channel1=channel1,channel2=channel2
	...
	if not keyword_set(called) then begin             ; Graphical interface
		...
		main = WIDGET_BASE(GROUP_LEADER=wid.base,row=4,TITLE='Interchannel phase extraction',/floating,/tlb_kill_request_events,/tlb_frame_attr)		
		...
		if matrixflag eq 0 then widget_control,butt2,GET_VALUE=channel2
...
; read / write header
	...
	srat,outputfile,eee,header=[2l,file.xdim,file.ydim,4l],info=info,type=302l
...
; pop up progress window
	...
	progress,Message='Calculating interchannel phase...',/cancel_button
...
; calculating span
	...
	for i=0,anz_blocks-1 do begin
		...
		if file.type eq 601 then begin
			oblock = atan(block[channel1 mod file.vdim,channel1 / file.vdim,*,*] * conj(block[channel2 mod file.vdim,channel2 / file.vdim,*,*]),/phase)
		endif else begin
			if matrixflag eq 0 then oblock = atan(block[0,channel1,*,*] * conj(block[0,channel2,*,*]),/phase)
			if matrixflag eq 1 then oblock = atan(block[channel1,channel2,*,*],/phase)
		endelse
		writeu,eee,oblock
	endfor
	free_lun,ddd,eee
