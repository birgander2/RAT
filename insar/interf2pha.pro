...
; RAT - Radar Tools
;------------------------------------------------------------------------
; RAT Module: interf2pha
; written by    : Andreas Reigber
; last revision : 14.Mar.2003
; Extracts interferometric phase from complex interferograms or image pairs
;------------------------------------------------------------------------
...
;start block processing
	...
	for i=0,anz_blocks-1 do begin   ; normal blocks
		...
		readu,ddd,block
; -------- THE FILTER ----------
		if file.type eq 300 then writeu,eee,atan(block[0,0,*,*] * conj(block[0,1,*,*]),/phase)
		if file.type eq 301 then writeu,eee,atan(block,/phase)
; -------- THE FILTER ----------
	endfor
	free_lun,ddd,eee
