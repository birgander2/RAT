...
;file = DIALOG_PICKFILE(/READ, FILTER = filters) 
     default_filters = ['*.rat;*.mrat','*.rat','*.mrat','*.rit']
     inputfile = cw_rat_DIALOG_PICKFILE(TITLE='Open RAT file', DIALOG_PARENT=wid.base, $
...
; read rit parameters (palette inclusive)
     open_rit,filename=inputfile
...
; 		cfile  = strmid(inputfile,0,strpos(inputfile,'.rat'))+'.pal'  ; optional palette file existing?
