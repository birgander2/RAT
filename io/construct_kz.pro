...
pro construct_kz, CONNECT=CONNECT, TRACKS_NR=cur_tr
  ...
  tryagain:
  main = WIDGET_BASE(GROUP_LEADER=wid.base,/column,TITLE='Construct vertical wavenumber (kz) file', $
  ...
  for i=0,cur_tr-1 do begin
     ...
     texts[i] = CW_FIELD(lines[i],VALUE=files[i],/NOEDIT,/string,XSIZE=50,TITLE='kz file for track '+strcompress(i,/r)+':')
  ...
  but_add  = cw_bgroup(main,label_left="Connect this kz-file with the current dataset? ",[' '],set_value=[connect],/nonexcl)
...
;	     WIDGET_CONTROL, main, /REALIZE,tlb_get_size=toto
  ...
  repeat begin                  ; Event loop
     ...
     if event.id eq but_info then begin ; Info Button clicked
        infotext = ['VERTICAL WAVENUMBER (kz) FILE CONSTRUCTOR',$
                    ' ',$
     ...
     for i=0,cur_tr-1 do begin
        if event.id eq brows[i] then begin ; Browse clicked
           path = config.workdir
           files[i] = cw_rat_dialog_pickfile(TITLE='Open RAT file with kz content', DIALOG_PARENT=wid.base, FILTER = '*.rat', $
  ...
  path = config.workdir
  sfile = cw_rat_dialog_pickfile(TITLE='Save kz file', DIALOG_PARENT=wid.base, FILTER = '*.rat', $
  ...
  newsiz = dialog_resize(title="Do you wish to resize the kz-file?",xdim=xdim,ydim=ydim,change=change,resize=resize)
  if change then begin
     kz=kz[*,newsiz[0,0]:newsiz[1,0],newsiz[0,1]:newsiz[1,1]]
     if resize then begin
        if total(((size(kz))[2:3]>newsiz[2,*])mod((size(kz))[2:3]<newsiz[2,*])) eq 0 $
        then kz=rebin(kz,n_tr,newsiz[2,0],newsiz[2,1]) $
        else kz=congrid(kz,n_tr,newsiz[2,0],newsiz[2,1])
     endif
  endif

  kz=reform(kz,/overwrite)
  if conjugate then $
     kz = -kz

  srat,sfile,kz,type=newtype,info=strcompress(n_dim,/r)+'-dim kz file for '+strcompress(n_tr,/R)+' tracks'
  if connect then begin
     ignore=set_par('kz_file',sfile)
     info = dialog_message("Don't forget to save your current dataset to save also the connection to the kz_file.",Dialog_parent=wid.base,/info)
  endif
end
