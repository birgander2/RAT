...
function subap_subapy, a, res=res, n_ap=n_ap, OUTPRINT=OUTPRINT, DORESIZE=DORESIZE, SIZ_RES=SIZ_RES,ERROR=ERROR, HAMMING=hamming, FLATTEN=flatten
...
;------------------------------------------------------------------------
  ...
  fl_y  = total(fl_y,1)/n[0]
	...
	siz_res = 2^(floor(alog(n_y / floor(100.0 / res))/alog(2))+1)  ; next higher 2^n
	stp_sub = floor(findgen(n_ap)*float(n_y-len_sub)/n_ap-1)+max(ind)
...
;;; - generate the new image output
  if keyword_set(DORESIZE) then newim = complexarr(n_ch,n_ap,n[0],siz_res) else newim = complexarr(n_ch,n_ap,n[0],n[1])
...
pro subap_generate_y,CALLED = called
  common rat, types, file, wid, config
...
; right type? --- calculate new type
  ...
  if not keyword_set(called) then begin ; Graphical interface
     main = WIDGET_BASE(GROUP_LEADER=wid.base,row=5,TITLE='Sub-aperture generation in y',/floating,/tlb_kill_request_events,/tlb_frame_attr)
...
; pop up progress window
  progress,Message='Generating subapertures...',/cancel_button

  block  = make_array([file.vdim,file.zdim,file.xdim,file.ydim],type=file.var)
  readu,ddd,block
  oblock = subap_subapy(block,res=res,n_ap=n_ap,DORESIZE=resize,SIZ_RES=siz_res,ERROR=ERROR,hamming=hamming,flatten=flatten)
  if keyword_set(ERROR) then return
  if keyword_set(RESIZE) then newdim[n_elements(newdim)-1] = siz_res
  srat,outputfile,eee,header=[newdim,file.var],info=info,type=newtype
  writeu,eee,oblock
  free_lun,ddd,eee
  oblock = 0.0
